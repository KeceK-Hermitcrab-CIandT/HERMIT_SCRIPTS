    ...
    public class MissingPrefabGuidBatchReplacer : EditorWindow
    {
        ...
        private void OnGUI()
        {
            ...
            for (int i = 0; i < missingPrefabs.Count; i++)
            {
                ...
                EditorGUILayout.ObjectField(data.missingObj, typeof(GameObject), true, GUILayout.Width(250));
                EditorGUILayout.LabelField("GUID: " + data.extractedGuid, GUILayout.Width(300));

                // Replacement prefab column
                data.replacementPrefab = (GameObject)EditorGUILayout.ObjectField("Replacement Prefab",
                    data.replacementPrefab, typeof(GameObject), false);
        ...
        private void ReplaceAllGuids()
        {
            foreach (var data in missingPrefabs)
            {
                ...
                Debug.Log(
                    $"✅ GUID of prefab {data.replacementPrefab.name} has been replaced with {data.extractedGuid}");
            }
